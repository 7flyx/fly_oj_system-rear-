...
public class RedisService {
    ...
    //************************ 操作Hash类型 ***************************
    public <T> T getCacheMapValue(final String key, final Long hKeyLong, Class<T> clazz) {
        return getCacheMapValue(key, String.valueOf(hKeyLong), clazz);
    }
     ...
     * @return Hash对象集合
     */
    public <T> List<T> getMultiCacheMapValue(final String key, final List<Long> hKeys, Class<T> clazz) {
        if (CollectionUtils.isEmpty(hKeys)) {
            return null;
        }
        Set<String> hkeysStrSet = hKeys.stream().map(hkey -> String.valueOf(hkey)).collect(Collectors.toSet());
        return getMultiCacheMapValue(key, hkeysStrSet, clazz);
    }
